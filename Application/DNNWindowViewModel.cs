...
namespace MainApp
{
    ...
    class DNNWindowViewModel
    {
        ...
        Tensor x, y; // Placeholders for intputs (x) and outputs(y)
        ...
        int filter_size2 = 5; // Convolution filters are 5 x 5 pixel
        ...
        public DNNConfig InitConfig()
            => Config = new DNNConfig
            {
                Name = "Custom DNN",
                Enabled = true,
        ...
        public void Train()
        {
            ...
            foreach(var epoch in range(epochs))
            {
                ...
                BlockingCollection<(NDArray c_x, NDArray c_y, int iter)> BlockC = new BlockingCollection<(NDArray c_x, NDArray c_y, int iter)>(TrainQueueCapa);
                Task.Run(() =>
                {
        ...
        (NDArray, NDArray) GetNextBatch(Session sess, string[] x, NDArray y, int start, int end)
        {
            ...
            {
                NDArray img4 = cv2.imread(x[i], SharpCV.IMREAD_COLOR.IMREAD_GRAYSCALE);
